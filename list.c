...
// Your freeList procedure should first free all nodes of the list including the
// sentinel node and finally free the list structure itself.
...
bool none(list *xs){
   if (xs->current->x != xs->none->x){
   return false;
}
   else if (xs->current->next != xs->none->back){
     return true;
   }
   else{
...
node *p = xs->current;
node *q = malloc(sizeof(node)); // Newnode
node *r = xs->current->next;
